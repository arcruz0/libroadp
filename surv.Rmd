...
```{r surv, echo=FALSE, out.width='70%', fig.cap="Ejemplos de observaciones presentes en una base de datos de supervivencia"}
knitr::include_graphics("00-images/surv_fig_10_1.PNG")
```
...
```{r weibull, echo=FALSE, fig.cap="Diferentes riesgos de base en el modelo de Weibull"}
knitr::include_graphics("00-images/surv_fig_10_2.png")
```
...
```{r echo=FALSE, fig.cap="Valores de poliarquía para Colombia según V-Dem"}
knitr::include_graphics("00-images/surv_fig_10_3.png")
```
...
```{block, type="books"}
Si tu base de datos está en este formato desde el comienzo, entonces podrás saltarte este paso.
```
...
```{r, fig.height=20, fig.width=7, fig.cap="Diagrama de Gantt para todas las observaciones"}
library(ggalt)
...
```{r, fig.cap="Diagrama de Gantt para América del Sur"}
...
```{r, fig.cap="Diagrama de Gantt para América del Sur con todas las mejoras"}
...
```{r, fig.cap="Diagrama de Gantt para todas las observaciones"}
library(ggplot2)
gantt_plot_sa <- gantt_plot_sa +
  labs(x     = "año", y = "",
...
```{r kaplan, fig.height=10, fig.width=11,  fig.cap="Curva de Kaplan-Meier"}
library(survminer)
...
- Utilizando `survminer`grafique una curva de Kaplan-Meier para la variable `c_gbr`.

- La variable `c_ussr`indica aquellos países que fueron parte de la Unión Soviética. Grafique una curva de Kaplan-Meier para la variable. Incorpore esta variable a un sexto modelo, haga su test de Grambsch y Therneau y rehaga la tabla de los modelos con `texreg`

- ¿Tenés tu propia base de datos de supervivencia? Sería genial que repitieras todo el ejercicio usando tus datos y compartas dudas en nuestro GitHub.
```
